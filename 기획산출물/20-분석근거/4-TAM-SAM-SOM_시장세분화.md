...
3. **1년차 목표** — SOM은 유료 가구 약 **440가구 / 매출 약 3,100만원**(월 5,900원 기준). 구독 단독으로는 규모가 나오지 않으므로 **결제 수수료·금융사 제휴를 포함한 수익모델 설계가 전제**다.
...
> ⚠️ **정정** — 이전 판에 적혀 있던 「디지털 용돈관리 앱 6.7조원 → 19.9조원, CAGR 12.8%」는 재조사 결과 **확인되지 않습니다.** 같은 조사기관(Dataintelo)의 실제 *Digital Pocket Money Kid App Market* 값은 **2.0조원 (2024) → 7.1조원 (2033), CAGR 16.7%** 로, 규모는 3분의 1 수준이고 성장률은 더 높습니다.
...
> 💡 **아이부자를 넣어도 비중이 거의 안 변한 이유** — 분자에 78만이 더해졌지만, 동시에 퍼핀·아이쿠카를 **자녀 기준으로 정정**(62만→32만)했기 때문입니다. 두 보정이 상쇄되어 이전 판(17.1%)과 사실상 같은 값(16.1%)이 나왔습니다. **추정치가 이 정도 보정에 견딘다는 것 자체가 신뢰도 근거**입니다.
...
> ⚠️ **단위 일치 확인** — 이전 판은 분자에 **부모 포함 가입자 62만**을, 분모에 **학생 수**를 넣어 단위가 섞여 있었습니다. [3-2](#3-2-게임화-비중을-직접-도출한다)에서 확립한 **자녀 계정 기준 110만**(아이부자 78만 + 아이쿠카 18만 + 퍼핀 14만)으로 정정했습니다.
...
> 위 세 근거 모두 **확인값**입니다. 이전 판은 「퍼핀 전체 매출 14억원」이라는 **미확인값**으로 같은 결론을 냈으나, 그 값을 빼도 결론은 그대로 성립합니다.
>
...
> ❌ **이렇게 쓰면 안 됩니다**
> ~~"한국 초등 저학년 금융교육 게이미피케이션 SOM = 35.6만명"~~ *(원자료 ChatGPT가 제시한 값)*
...
- [`../gpt_deep_research_키즈 금융교육·행동 보상 시장.md`](../../%EC%A1%B0%EC%82%AC/%EC%9C%A0%EB%A6%BC/gpt_deep_research_%ED%82%A4%EC%A6%88%20%EA%B8%88%EC%9C%B5%EA%B5%90%EC%9C%A1%C2%B7%ED%96%89%EB%8F%99%20%EB%B3%B4%EC%83%81%20%EC%8B%9C%EC%9E%A5.md) — 심층 리서치 원문
