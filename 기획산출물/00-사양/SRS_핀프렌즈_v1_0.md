...
- **문서** — `기획산출물/00-사양/SRS_핀프렌즈_v1_0.md` · **버전** v1.0 · **작성일** 2026-09-01
- **표준** ISO/IEC/IEEE 29148:2018 (Systems and software engineering — Life cycle processes — Requirements engineering)
- **상위 문서** [`PRD_핀프렌즈_v0_3.md`](PRD_%ED%95%80%ED%94%84%EB%A0%8C%EC%A6%88_v0_3.md) · [`기능정의_v14.md`](%EA%B8%B0%EB%8A%A5%EC%A0%95%EC%9D%98_v14.md)
- **지식베이스** [`wiki/`](../../wiki/index.md) — 각 결정의 근거는 해당 페이지의 **「🧭 결정 배경 (Context)」** 절
- **작성** 하영 · **제품기획** 팀 공동
- **역할** 사양·지식베이스 하영 / 학습·기술 설계 수빈 / 정책·법령·구현 병윤 / 고객·서비스 분석 혜원
...
PRD v0.3 · 기능정의 v14 · 학습 난이도 5단계 설계안 · 퀴즈 샘플 60문항 · 초등저학년 예적금 비교 · 개인정보보호법 §22의2 · 전자금융거래법 · 위치정보법
...
| CON-08 | LLM 호출은 **서버 경유 · 국내 리전**이며 **아동 식별 데이터를 페이로드에 넣지 않는다** | NFR-009·013 · §2.5.2 |
...
### 2.5 기술 스택 (Technical Stack)

> 🔴 **스택은 요구사항이 아니라 제약이다.** 본 절의 목적은 *"무엇을 쓰는가"* 를 고정하는 것이 아니라,
> **§2.3 제약과 §5.1 규제 요구사항이 스택 선택에 어떻게 걸리는지**를 검증 가능한 형태로 남기는 것이다.
> 각 항목은 그것을 강제·제약하는 요구사항과 함께 적는다. **요구사항이 바뀌면 스택도 재검토 대상이다.**

#### 채택 (Adopted)

| 계층 | 스택 | 무엇이 이것을 강제·제약하는가 |
|---|---|---|
| **전달** | **PWA(웹)** · 부모/아이 **오리진 분리** | `PRD ADR-005` · `NFR-003`·`NFR-004` — 번들과 토큰 스코프를 **오리진 단위로 검증**한다 |
| **프런트엔드** | **Next.js** (TypeScript · React) | 한 코드베이스에서 **두 오리진**을 낸다 — 코드베이스는 하나, **빌드·배포 타깃은 아이/부모 2개**다. 🔴 `NFR-008` — 런타임 제3자 오리진 호출 **0건**이어야 하므로 폰트·아이콘·라이브러리를 **전부 자체 호스팅**한다(외부 CDN 및 원격 폰트 로딩 금지) |
| **3D 렌더링** | **three.js** (glTF 에셋) | `기능정의 v14 §16` · §9-1 아바타·공간 키트·펫. 🔴 `NFR-030`(p95 ≤ 800 ms) — 저사양 Android에서 경합하므로 `FR-034` 유휴 애니메이션은 **CSS transform 기반**, JS 애니메이션 루프 금지 |
| **오프라인** | **Service Worker** | `NFR-022` 오프라인 후 재연결 시 ⭐ **중복 지급 0건** — 오프라인 큐와 `idempotency_key` 생성을 여기서 구현한다 |
| **백엔드** | **Next.js 서버 런타임** — Route Handler · Server Action (TypeScript) | `PRD ADR-018` — 프런트와 **런타임을 하나로 유지**한다. 4인 팀이 배포·인증·타입 정의를 두 벌 관리하지 않는다. 🔴 `NFR-003` — 서버 전용 모듈은 `server-only`로 표시해 **아이 번들에 부모 로직이 섞이면 빌드가 깨지게** 한다. 계약 수준 명세는 `기획산출물/10-기술설계/API 명세` |
| **주 저장소** | **PostgreSQL** | `NFR-020` 별 원장 정합성 **오류율 0%** — **이중 기입 + 일일 정산 배치**가 트랜잭션 보장을 전제한다. `DR-01`~`DR-07`의 **컬럼 부재 제약**이 스키마 검수 대상 |
| **캐시 · 세션** | **Redis** | 🔴 `NFR-011` 결제 내역 **서버 사본 미보관**(캐시는 세션 범위) — **영속화를 끄고**(AOF·RDB 미사용) 세션 범위를 넘겨 보관하지 않는다. `DR-03`과 함께 검수 |
| **인프라** | **AWS**(ECS · RDS · S3) · **Docker** | 🔴 `NFR-009` 아동 데이터 **국외이전 없음** → **서울 리전(ap-northeast-2) 고정** · 리전 간 복제·백업 금지. 🔴 `NFR-005`·`DR-02` — **S3에 미션 이미지를 두지 않는다.** S3 용도는 **정적 자산·glTF 에셋**에 한정 |
| **CI/CD** | **GitHub Actions** | `NFR-003` 아이 오리진 번들 정적 스캔을 **배포 차단 게이트**로 넣는다(§7.2) |
| **관측** | **자체 호스팅 · 국내** | `NFR-008`·`NFR-009` — 외부 SaaS 관측·분석 도구를 쓰지 않는다 |
| **알림** | **Web Push**(VAPID) · `web-push` | `FR-022` [[계획-카드-작성-트리거]] · `NFR-024` 도달률 ≥ 70% · `NFR-044`. [[PWA]]에서 유일한 경로다. 🔴 푸시 엔드포인트는 **브라우저 벤더 서버**(국외)이므로 `NFR-009`·`G-2`와 같은 기준을 적용한다 — **페이로드에 아동 식별 데이터·계획 카드 내용을 넣지 않고** 본문은 고정 문구로 한다 |
| **AI 런타임** | **AWS Bedrock** (Anthropic Claude · **서울 리전**) | 🔴 `NFR-009` — 국내 리전에서 추론이 끝나야 국외이전이 발생하지 않는다. 사용처는 **`FR-042` 하나**. `NFR-013`·`NFR-014` 적용. ⬜ 서울 리전 모델 가용성 **확인 필요** |
| **AI 저작 파이프라인** | **Python · FastAPI** · **ChromaDB**(자체 호스팅) | **제품 요청 경로 밖**의 오프라인 배치다. 아동 데이터를 다루지 않아 `NFR-009` 대상이 아니다 |

#### 2.5.1 AI 스택 — **아동 데이터 접촉 여부**로 세 평면을 가른다

> 🔴 **분기 기준은 하나다 — 그 호출이 아동 데이터를 만지는가.**
> 만지지 않으면 규제 대상이 아니고, 만지면 **국내 리전 · 비식별 · 서버 경유** 셋을 모두 지켜야 한다.

| 평면 | 아동 데이터 | 스택 | 무엇에 쓰나 | 상태 |
|---|:-:|---|---|:-:|
| **D. 개발** | ❌ 없음 | LLM 코딩 도구 *(Claude Code 등)* | 구현·리팩터링·테스트 작성. **제품 런타임 아님** | ✅ |
| **A. 콘텐츠 저작** *(오프라인)* | ❌ 없음 | **Claude / OpenAI API** + **ChromaDB** + **Python·FastAPI** | `FR-011` 학습 지문·문항 생성. ChromaDB로 **기존 문항 임베딩과 대조**해 중복·난이도 드리프트를 잡는다 | ✅ |
| **R. 제품 런타임** | ✅ 있음 | **AWS Bedrock**(Claude · 서울 리전) | **`FR-042` 월간 요약 문장** — 부모 화면 전용 | ✅ |

**왜 ChromaDB인가** — 교안이 제시한 두 후보 중 **ChromaDB는 자체 호스팅**이 되고 **Pinecone은 국외 SaaS**다.
저작 파이프라인은 아동 데이터를 다루지 않지만, **인프라를 국내로 통일**하면 `NFR-009` 검수 범위가 단순해진다.

**왜 API 직접 호출이 아니라 Bedrock인가** — 런타임은 아동 데이터를 만진다.
벤더 API를 직접 부르면 요청이 **국외로 나가 `NFR-009`가 곧바로 깨진다.**
이미 AWS를 쓰고 있으므로 **같은 리전(서울) 안에서 추론을 끝내는 경로**가 가장 짧다.

#### 2.5.2 AI 가드레일 — 지키지 못하면 기능을 끈다

| ID | 규칙 | 검증 |
|---|---|---|
| **G-1** | 브라우저에서 **LLM API를 직접 호출하지 않는다** — 반드시 서버 경유 | `NFR-008`(제3자 오리진 0건) · CSP |
| **G-2** | LLM 요청 페이로드에 **아동 식별 데이터를 넣지 않는다** — 집계 수치와 라벨만 | `NFR-013` |
| **G-3** | **아동에게 생성 텍스트를 실시간으로 노출하지 않는다** — 저작물은 **사람 검수 후 정적 배포** | `NFR-014` |
| **G-4** | **미션 사진을 LLM에 보내지 않는다** | `P-13` · `NFR-005` · `DR-02` |
| **G-5** | **아바타·펫은 생성형 AI로 만들지 않는다** — 사전 제작 에셋에서 고른다 | `P-13` |
| **G-6** | 추론은 **서울 리전**에서 끝난다 · 요청 데이터를 **모델 학습에 제공하지 않는다** | `NFR-009` |

> 🔴 **G-3이 이 설계의 급소다.** 1·2단계 학습 콘텐츠는 금감원 대조 기준선이 없어(§9 미해결)
> **생성물 품질을 기계적으로 판정할 수단이 없다.** 그래서 저작은 **오프라인 + 사람 검수 게이트**로만 돌린다.

#### 2.5.3 범위 밖 (Out of scope)

| 후보 | 왜 넣지 않는가 |
|---|---|
| **벤더 API 직접 호출**(OpenAI·Anthropic 엔드포인트) — **런타임 한정** | 요청이 국외로 나가 `NFR-009` 위반. **저작 평면(A)에서는 사용한다** |
| **Express 별도 서버** | 런타임이 둘로 늘어 배포·인증·타입 정의가 **두 벌**이 된다. `NFR-003`·`NFR-004`는 프레임워크가 아니라 **빌드 산출물과 토큰 스코프**에 걸리므로 분리해서 더 얻는 보증이 없다(`ADR-018`) |
| **Pinecone** | 국외 SaaS — ChromaDB 자체 호스팅으로 대체 |
| **아동 대상 실시간 생성 텍스트** *(오답 해설·대화형 튜터 등)* | `G-3`. 오답 시 제공은 `FR-011`의 **「이야기 다시 읽기」** 로 고정 |
| **생성형 AI 아바타** | `P-13`이 명시적으로 금지 |
| **미션 사진 AI 판정** | `G-4` · 판정 주체는 부모(`FR-032`) |

#### 2.5.4 스택이 바뀌어도 남는 검증 항목

`CON-03`(전환 경로를 코드에 두지 않는다) · `NFR-005`(스토리지 내 미션 이미지 0건) ·
`NFR-008`(제3자 오리진 요청 0건) · `NFR-009`(국외이전 없음) · `NFR-020`(별 원장 정합성 0%) ·
`DR-01`~`DR-07`(컬럼 부재 제약).

> **벤더·프레임워크를 교체해도 위 항목은 그대로 남는다.** 스택 변경 제안은 이 목록을
> **어떻게 계속 만족시키는지**와 함께 와야 한다.
...
sequenceDiagram
    ...
    alt 승인
        S-->>CA: ⭐1 · 💵 용돈 충전 · practice_credited
    else 72h 미판정
        S->>S: 자동 지급 (auto=true)
        S-->>CA: ⭐1 · 💵 용돈 충전 · practice_credited
        Note over CA: 표시는 「확인하지 못했어요」<br/>지급하되 부모의 미확인을 감추지 않는다
    end
```
...
#### FR-032 미션 승인 · 사진 확인 · `PRD ADR-007` `PRD ADR-016` `US-6`

| | |
|---|---|
| **입력** | 미션 완료 제출(선택적 사진 첨부) |
| **처리** | 제출 즉시 부모에게 **승인 요청 알림을 1회** 발송하고 부모 화면 **승인 대기 목록**에 올린다. 부모 승인 화면에 사진을 표시하고 **판정 즉시 이미지를 파기**한다. 승인 시 ⭐1 + 보상 금액 충전 + `practice_credited`. **72h까지 부모 판정이 없으면 자동 지급**한다 |
| **출력** | 승인 요청 알림, 24h 리마인드, 부모 화면 승인 대기 목록 항목, 승인 이력(**`auto` 플래그** · 사유 텍스트 포함), ⭐, 지갑 충전, `mission_auto_credited` |
| **예외** | ① **24h 미승인 → 리마인드 1회** ② **72h 미승인 → 자동 지급** — **⭐1 + 💵 보상 금액 충전 + 「벌기」 실천 +1**을 모두 집행하고, 아이 화면·이력에 **「확인하지 못했어요」**(`auto=true`)로 표시한다. 부모 지갑 잔액이 부족하면 **충전만 보류**(FR-033 예외 ②) ③ 반려 → 「승인되지 않음 + 사유」를 **「미실천」과 시각적으로 구별** ④ 대기 5건 이상 → 일괄 승인 경로 제공 |

> 🔴 **병목의 비용을 아이에게 물리지 않는다.** 승인은 아이가 통제할 수 없는 변수(부모 폰·습관·푸시 도달률)에
> 걸려 있고, 미션은 트리거 8종 중 **유일한 수동**이라 이 하나만 부모 사정으로 사라진다.
> 자동 지급은 새 약속이 아니라 **부모가 등록 시 정한 보상 금액을 그대로 집행**하는 것이다.

> 🔴 **지급하되 라벨은 분리한다.** 자동 지급 건은 **「칭찬받았어요」가 아니라 「확인하지 못했어요」** 로 표시하고
> 이력에 `auto=true` 를 남긴다. **부모 확인 없이 실제 돈이 움직이는 유일한 경로**이므로,
> `mission_auto_credited` 비율(NFR-025)을 감시하고 기준 초과 시 `PRD ADR-016`의 되돌리는 조건이 발동한다.
...
- **AC-032-3** **Given** 72시간 부모 판정 없음 / **When** 자동 지급 배치가 실행된다 / **Then** **⭐1 · 보상 금액 충전 · 「벌기」 실천 +1**이 모두 집행되고, 아이 화면에 **「확인하지 못했어요」** 가 표시되며 이력에 **`auto=true`** 로 기록된다.
...
- **AC-032-6** **Given** 자동 지급된 건 / **When** 부모가 승인 대기 목록·이력을 연다 / **Then** **「확인하지 못했어요」가 수동 승인(「칭찬받았어요」)과 시각적으로 구별**되어 표시된다.
- **AC-032-7 (실패)** **Given** 자동 지급이 끝난 건 / **When** 부모가 반려를 시도한다 / **Then** **반려가 불가능**하고(승인 후 취소와 동일 원칙), 대신 **해당 미션 비활성화** 경로가 제공된다.
- **AC-032-8** **Given** 자동 지급 건이 포함된 주 / **When** WPA를 산출한다 / **Then** 해당 건은 **분자에 포함**되되 `auto=true` 로 **별도 집계**되어 자동 지급 비율을 산출할 수 있다.
- **AC-032-9 (실패)** **Given** 자동 지급 시점에 부모 지갑 잔액이 보상 금액보다 적다 / **When** 배치가 실행된다 / **Then** **⭐와 실천은 지급**되고 **충전만 보류**되며 부모에게 충전 요청이 표시된다.
...
| **예외** | ① **보상 금액 0원 미션 허용** — ⭐만 지급 ② 부모 지갑 잔액 부족 → 승인은 되되 **충전은 보류**되고 부모에게 충전 요청이 표시된다. ⭐와 실천은 **정상 지급**된다 ③ 승인 후 취소 → **⭐와 충전 모두 회수하지 않는다**(아이가 이미 받은 것을 잃지 않는다) ④ β 기간에는 🧪 **모의 충전**으로 처리된다 ⑤ **72h 자동 지급**(FR-032) → **⭐·실천·충전을 모두 집행**한다. 충전액은 **부모가 등록 시 정한 보상 금액** 그대로이며 시스템이 산정하지 않는다 |
...
---
#### FR-042 월간 요약 문장 (AI) · `PRD ADR-017` `US-1`

**AI가 제품 런타임에 들어오는 유일한 지점이다.** 부모 화면 전용이며, 아이는 이 문장을 보지 않는다.

| | |
|---|---|
| **입력** | `FR-040`이 산출한 **전월 대비 변화 지표**(집계 수치) · 아동 **닉네임 아님 · 식별자 아님**의 라벨(「아이」) |
| **처리** | **AWS Bedrock**(서울 리전)에 집계 수치만 전달해 **2~3문장 요약**을 생성한다. 실패·지연 시 **템플릿 문장으로 대체**한다 |
| **출력** | 월간 숲 상단의 요약 문장 · `monthly_summary_generated` |
| **예외** | ① 생성 실패·타임아웃(**≤ 3초**) → **규칙 기반 템플릿 문장** ② 첫 달 → `FR-040` 예외를 따른다 ③ 실천 0건 → 생성하지 않고 첫 실천 안내를 쓴다 |

**왜 이 자리인가** — [[정미경-H1]]의 *"보고도 모른다"* 가 이 제품의 출발점이고,
[[월간-숲]]은 **전월 대비 델타가 성장 증거**라는 전제 위에 서 있다. 그 델타를 **문장으로 옮기는 것**이
숫자를 읽지 못하는 부모에게 가장 직접적인 보완이다.

- **AC-042-1** **Given** 2개월 이상 사용한 계정 / **When** 숲을 연다 / **Then** 변화 지표를 근거로 한 요약 문장이 **3초 이내** 표시된다.
- **AC-042-2 (실패)** **Given** Bedrock 호출 실패 또는 3초 초과 / **When** 화면을 연다 / **Then** **템플릿 문장이 대신 표시**되고 화면은 정상 동작한다(오류 노출 0건).
- **AC-042-3 (실패)** **Given** 생성 요청 페이로드 / **When** 로그를 감사한다 / **Then** **아동 식별 데이터가 0건**이다(`NFR-013`).
- **AC-042-4 (실패)** **Given** 아이 오리진 / **When** 번들·네트워크를 검사한다 / **Then** **요약 문장 API 호출이 0건**이다 — 아이 화면에 노출되지 않는다.
- **AC-042-5** **Given** 브라우저 네트워크 로그 / **When** 검사한다 / **Then** **LLM 엔드포인트로 나가는 직접 호출이 0건**이다(`G-1`·`NFR-008`).

> 🔴 **이 기능은 언제든 끌 수 있어야 한다.** 요약 문장이 없어도 `FR-040`의 변화 지표는 그대로 성립한다 —
> **AI는 보완이지 전제가 아니다.** 가드레일(§2.5.2) 중 하나라도 지키지 못하면 기능을 끄고 템플릿으로 되돌린다.
...
| **NFR-013** | LLM 요청 페이로드 내 **아동 식별 데이터** | **0건** | 요청 로그 표본 감사(주 1회) · 스키마 화이트리스트 자동 테스트 |
| **NFR-014** | 아동에게 노출되는 **생성 콘텐츠의 사전 사람 검수** | **100%** | 배포 전 검수 서명 · 미검수 콘텐츠 배포 시 파이프라인 차단 |
...
| **NFR-025** | 미션 **자동 지급률** `mission_auto_credited / mission_submitted` | **≤ 10%** `[가설]` | 주 단위 집계. 🔴 이 값이 곧 **부모가 아이의 실천을 한 번도 안 본 비율**이며, **부모 확인 없이 나간 돈의 비율**이기도 하다. 초과 시 `PRD ADR-016` 되돌리는 조건 발동 |
...
`quiz_submitted` · `mission_registered` · `mission_submitted` · `mission_auto_credited` · `mission_reward_paid` · `monthly_summary_generated`(`auto` 실패 시 `fallback=true`) · `wallet_topped_up`(source: parent | mission) · `plan_card_created` ·
...
| **OI-09** | ⬜ **AWS Bedrock 서울 리전의 Claude 모델 가용성** 미확인 — 불가 시 `FR-042`는 템플릿 문장으로만 동작한다 | 스택 확정 |
