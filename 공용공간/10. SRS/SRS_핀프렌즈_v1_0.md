...
#### 범위 밖 (Out of scope) — `PRD ADR-017`

| 후보 | 왜 넣지 않는가 |
|---|---|
| **OpenAI / Claude API** | ① **런타임 LLM 호출이 필요한 기능이 없다** — 학습 지문·퀴즈 60문항은 사전 제작이고(`FR-011`), 미션 판정은 부모(`FR-032`), ⭐ 판정은 **금액 비교**다(`FR-021`·`DR-06`) ② 아동 데이터를 국외 API로 보내면 `NFR-009` **직접 위반** ③ 아바타는 `P-13`이 **생성형 AI 미사용**을 명시한다 |
| **Vector DB** (ChromaDB · Pinecone) | 위 ①에 종속 — 검색·RAG 대상 코퍼스가 없다. Pinecone은 **국외 SaaS**라 `NFR-009`에도 걸린다 |
| **Python · FastAPI** | **AI/ML 연동 전용**으로 제안된 런타임이라 위 둘이 빠지면 도입 근거가 없다. 4인 팀에서 런타임을 둘로 늘리는 비용만 남는다 |

> 🔑 **LLM을 쓰는 자리가 하나 있고, 그것은 런타임이 아니라 콘텐츠 저작이다.**
> 지문·문항을 만드는 **사내 저작 도구**로 쓰는 것은 아동 데이터가 나가지 않으므로 위 제약과 무관하다.
> **제품 스택이 아니므로 본 절에 포함하지 않는다.**
...
`quiz_submitted` · `mission_registered` · `mission_submitted` · `mission_auto_credited` · `mission_reward_paid` · `wallet_topped_up`(source: parent | mission) · `plan_card_created` ·
