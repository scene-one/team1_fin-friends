...
| **백엔드** | **Next.js 서버 런타임** — Route Handler · Server Action (TypeScript) | `PRD ADR-018` — 프런트와 **런타임을 하나로 유지**한다. 4인 팀이 배포·인증·타입 정의를 두 벌 관리하지 않는다. 🔴 `NFR-003` — 서버 전용 모듈은 `server-only`로 표시해 **아이 번들에 부모 로직이 섞이면 빌드가 깨지게** 한다. 계약 수준 명세는 `공용공간/11. 기술기획 산출물/비즈니스 로직/API 명세` |
