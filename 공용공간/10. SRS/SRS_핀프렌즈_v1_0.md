...
| **프런트엔드** | **Next.js** (TypeScript · React) | 한 코드베이스에서 **두 오리진**을 낸다. 🔴 `NFR-008` — 런타임 제3자 오리진 호출 **0건**이어야 하므로 폰트·아이콘·라이브러리를 **전부 자체 호스팅**한다(외부 CDN 및 원격 폰트 로딩 금지) |
...
| **백엔드** | **Node.js · Express** (TypeScript) | 프런트와 언어를 일치시켜 4인 팀의 런타임을 **하나로 유지**한다. 계약 수준 명세는 `공용공간/11. 기술기획 산출물/비즈니스 로직/API 명세` |
