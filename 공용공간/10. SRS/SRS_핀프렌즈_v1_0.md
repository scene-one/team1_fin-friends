...
sequenceDiagram
    ...
    Note over S,P: 중간 리마인드 없음 — 알림은 이 한 번뿐
    P->>S: 칭찬하기 / 반려하기
...
| **출력** | 승인 요청 알림 **1회**, 부모 화면 승인 대기 목록 항목, 승인 이력(사유 텍스트 포함), ⭐ |
| **예외** | ① **72h 미승인 → 자동 만료**, ⭐ 미지급 ② 반려 → 「승인되지 않음 + 사유」를 **「미실천」과 시각적으로 구별** ③ 대기 5건 이상 → 일괄 승인 경로 제공 |

> 🔴 **부모에게 가는 알림은 제출 즉시 1회뿐이다.** 만료 전 중간 리마인드가 없으므로
> **이 알림 한 번이 승인 병목 전체를 떠받친다.** 미도달분의 대체 경로는 **부모 화면의 승인 대기 목록**뿐이고,
> 그것은 부모가 앱을 열어야 작동한다 — FR-022가 알림·애니메이션 두 겹을 두는 것과 달리 **여기는 한 겹이다.**
> `mission_expired` 비율(NFR-025)로 감시하고, 기준을 넘으면 `PRD ADR-016`의 되돌리는 조건이 발동한다.
...
- **AC-032-5 (실패)** **Given** 제출 후 **24~71시간** 미승인 상태 / **When** 알림 배치가 실행된다 / **Then** 그 건에 대한 **추가 알림 발송이 0건**이다.
...
| **NFR-025** | 미션 승인 만료율 `mission_expired / mission_submitted` | **≤ 10%** `[가설]` | 주 단위 집계. 🔴 **승인 요청 알림은 제출 즉시 1회뿐**이라 이 값이 곧 알림 실패율이다. 초과 시 `PRD ADR-016` 되돌리는 조건 발동 |
