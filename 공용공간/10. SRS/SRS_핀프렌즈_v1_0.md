...
| 구분 | 내용 |
|---|---|
| **In** | 학습·퀴즈(3단계) · **미션 루프(용돈 보상 + ⭐)** · **봉투형 소비 관리** · 위시리스트 · **부모 적금 신청·협상** · 별 지급 · 아바타/옷장 · 성장 나무 · 월간 숲 · 소비 내역 · 부모 온보딩·법정대리인 동의 · **아이 앱 페어링** |
...
> ⭐는 옷장에서 쓰는 아이의 재화이고, 용돈은 봉투에 나눠 담아 실제로 쓰는 돈이다.
...
| | *(수행)* 완료 제출(선택적 사진 첨부) |
| **처리** | 부모 승인 시 ① **보상 금액을 아이 지갑에 충전**하고 ② **⭐1**을 지급하며 ③ `practice_credited`(벌기)를 기록한다. 충전 완료 시 **봉투 배분 화면으로 연결**한다(FR-020) |
...
- **AC-033-2** **Given** 미션 보상 충전이 완료 / **When** 아이가 앱을 연다 / **Then** **봉투 배분 화면이 열린다**(FR-020 · O7 분모에 포함).
...
`quiz_submitted` · `mission_registered` · `mission_submitted` · `mission_expired` · `mission_reward_paid` · `wallet_topped_up`(source: parent | mission) · `envelope_allocated` ·
...
| US-2 첫 실천이 나무에 반영된다 | FR-010 · FR-011 · **FR-033** · FR-030 | NFR-020 · NFR-030 | 게이트 |
