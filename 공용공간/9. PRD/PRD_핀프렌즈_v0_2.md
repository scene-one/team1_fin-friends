...
> 1. **선별 Pain 3건 중 1건(C5 3분 미달 사각지대)의 대응 기능이 MVP에 없습니다.** 숨기지 않고 §1-4·§4-3·§7-2에 남겼습니다.
...
| ★ **선별** | **C5(3분 미달 사각지대)** | **3분 미달 사각지대** | 「3분 체류」 트리거가 1분 미만 즉흥 구매를 못 잡는다 | 배성우 |
...
| △ **감시** | **C6(회고 형식화)** | **회고 형식화** | 회고를 읽지 않고 [확인했어요]만 눌러도 ⭐1이 들어온다 | 배성우 |
...
| **E1~E11** | **Experiment** — 검증 실험 | E3 = 체류시간 스톱워치 실측 |
| **F1(성장 나무)~F19(소비 사전 개입 2차 트리거)** | **Feature** — 기능 번호 *(기능정의 v13·가치제안서 승계)* | F19 = 소비 사전 개입 2차 트리거 |
| **US-1~US-8** | **User Story** — 사용자 스토리 | US-2 = 첫 실천이 나무에 반영된다 |
| **AC / AC-E** | **Acceptance Criteria** — 수용 기준 / **AC-E** = 예외·실패 케이스 기준 | US-6 AC-E2 |
| **P-01~P-22 · F-01** | **정책·법령 조항** — 규제 상수 | P-19 = 만 14세 미만 위치정보 옵트인·좌표 미저장 |
...
| ★ **선별** | **C5(3분 미달 사각지대)** | 「3분 체류」가 1분 미만 즉흥 구매를 못 잡는다 | **소비 순간 트리거 발동률 < 50%**(대상 소비 건 대비) · 아이 소비 중 앱 기록 비율 **< 70%** | **0** *(현금 = 기록 0건)*<br>🔴 **체류시간 실측 선행 필수** | ✍️[합성 가설] | ⚠️ **부분** — §1-4 |
...
| △ **감시** | **C6(회고 형식화)** | 회고를 읽지 않고 [확인했어요]만 눌러도 ⭐1이 들어온다 | **동일 회고 문장 재노출률 > 50%**(7일 창) · 회고 화면 체류 **중위 < 2초** | ⬜ 미측정 | [설계 확정]+[추론] | ✅ |
...
| O7 | 쓰기 전에 멈춘 기록이 남는다 | 사전 개입 후 「남기기」 선택 주당 횟수 | **0회** | **주 2회** | 주간 | 🔴 **F19(소비 사전 개입 2차 트리거) 신설 전제** |
...
| `retro_viewed` | 회고 화면 진입~이탈 | `child_id` `sentence_id` `dwell_ms` `plan_amount` `actual_amount` | △ 감시 C6(회고 형식화) 체류 중위 · 동일 문장 재노출률 |
...
| **WPA-v2** *(개통 후)* | v1 + **트리거 7·8** 예적금(F15) + **F19(소비 사전 개입 2차 트리거)** 사전 개입 — **5종** | F15·F19 개통 시점부터 |

> 🔴 **v0.1의 정의는 5종을 분자에 넣었으나, 그중 예적금(F15(예적금 실천) Could)과 사전 개입(F19(소비 사전 개입 2차 트리거) Should)은 MVP에 없습니다.** 정의와 구현이 어긋나면 **WPA가 구조적으로 3종 상한에 묶인 것을 성능 문제로 오독**하게 됩니다.
...
| 미대응 | 내용 | 왜 |
|---|---|---|
| ★ **C5(3분 미달 사각지대)** *(선별)* | 소비 순간 사전 개입 — **AOS 4.0 전체 공동 1위** | 대응 기능 **F19(소비 사전 개입 2차 트리거)가 MVP 밖**. F14(3분 체류 위치 알림)를 올려도 **3분 사각지대가 곧 C5(3분 미달 사각지대)**라 해결되지 않음 |
| ◇ **C10(FOMO 3초 판단) · C12(온라인 개입 부재)** *(보조)* | 온라인 결제 사전 개입 | 동일 — F19(소비 사전 개입 2차 트리거) 하나에 세 Pain이 함께 걸림 |
...
> **결과** — MVP는 **선언 ②(그 성장이 보인다)를 완성하고 선언 ①(성장이 일어난다)을 4칸 중 3칸에서 개통**하는 범위입니다. *"소비 순간에 멈춘다"* 는 **로드맵 1순위(F19 소비 사전 개입 2차 트리거)** 로 남습니다.
...
> **선별 단위는 Pain Point이고 인물은 그 결과**입니다 — 선별 C1(성장 증거 부재)·C2(실천 공백 → 나무 정체)·C5(3분 미달 사각지대)를 고른 뒤 인물을 추적하니 H1·H2가 나왔습니다. **새 페르소나를 추가하지 않습니다.**

| | **H1 정미경** *(선별 — C1 성장 증거 부재 · C2 실천 공백→나무 정체 / 보조 — C3 정체 원인 미설명)* | **H2 배성우** *(선별 — C5 3분 미달 사각지대 / 감시 — C6 회고 형식화)* |
|---|---|---|
...
> 🔴 **선별 인물 둘 다 「전용 스마트폰을 들고 다니는 아이」가 아닙니다**(H1 태블릿=집 안 · H2 키즈워치). C5(3분 미달 사각지대)·C10(FOMO 3초 판단)·C12(온라인 개입 부재)가 **전부 이 전제 위에** 있습니다 — §7-3 의존성.
...
| **6** 실천 선택 | ★**C2(실천 공백 → 나무 정체)** 실천 칸이 비어 **4칸 전부 정체** *(감정 4)* | ★**C5(3분 미달 사각지대)** 자판기 30초 · 팽이 1분 → **3분 트리거 사각지대** *(감정 4)* | F2(미션 루프) · F12(위시리스트) / 🔴 **F19(소비 사전 개입 2차 트리거)(✖)** |
| **7** 🔑 실천 판정 | — | △**C6(회고 형식화)** 안 읽고 [확인했어요] → 참은 날과 안 참은 날이 같음 | F8(결제 회고) |
...
### US-4 — 🔴 쓰기 직전에 한 번 멈춘다 *(MVP 미대응)*  `J1 실천할 자리 만들기`  `★선별 C5 3분 미달 사각지대`
> **As a** 배주안(아이), **I want** 지갑을 여는 그 순간에 한 번 멈출 계기가 있고, **so that** 오늘 참았다는 기록이 남는다.
> 🔴 **이 스토리의 대응 기능(F19 소비 사전 개입 2차 트리거)은 MVP 밖입니다.** 아래 AC는 **로드맵 수용 기준**이며, MVP 기간에는 **AC0(선행 실측)만** 수행합니다.

| # | Given / When / Then | 측정 도구 · 표본 |
|:-:|---|---|
| **AC0** 🔴 *(MVP 내)* | **Given** H2형 아이의 하교 동선 3곳 이상 / **When** 실제 체류 시간을 스톱워치로 실측한다 / **Then** **가게별 체류 분포와 3분 초과 비율이 산출**되고, **C5(3분 미달 사각지대)의 발생률 0.6이 재산출**된다 | **스톱워치 실측** · 동선 3곳 × 5회 이상 |
| **AC1** *(F19(소비 사전 개입 2차 트리거) 전제)* | **Given** 여정 6단계, 계획 금액이 설정된 아동 / **When** 소비 직전 시점에 도달한다 / **Then** **대상 소비 건 중 트리거 발동률 ≥ 85%** · 체류 시간 조건에 의존하지 않는다 | 인앱 이벤트 + 실측 대조 |
| **AC2** *(F19(소비 사전 개입 2차 트리거) 전제)* | **Given** 트리거 발동 / **When** 아이가 「남기기」를 선택한다 / **Then** **「참음」이 자동 기록**되고 ⭐가 지급되며, **주당 「남기기」 선택 ≥ 2회** | 인앱 이벤트 |
| **AC3** *(F19(소비 사전 개입 2차 트리거) 전제)* | **Given** 아이 기기가 **키즈워치 또는 부모 폰 공유** / **When** 트리거 조건이 성립한다 / **Then** **알림 수신 가능 여부가 판정**된다 — 불가하면 해당 가정은 **대체 경로로 분기** | 🔴 **실기기 기술 검토** |
| 🆕 **AC-E1** *(F19(소비 사전 개입 2차 트리거) 전제)* | **Given** 계획 금액이 **미설정** / **When** 트리거 조건에 도달한다 / **Then** **트리거를 발동하지 않고** 계획 설정을 유도한다 · **오탐 발동 0건** | 단위 테스트 |
| 🆕 **AC-E2** *(F19(소비 사전 개입 2차 트리거) 전제)* | **Given** 아이 기기에서 **알림 권한이 거부**됨 / **When** 트리거 조건에 도달한다 / **Then** **앱 내 배너로 대체 노출**되고, **권한 거부 상태가 계측**된다 *(대상 범위 재산출 입력값)* | 권한 상태 로그 |
...
> **기능을 새로 발명하지 않았습니다.** VPS §8-2의 F1(성장 나무)~F19(소비 사전 개입 2차 트리거)를 그대로 승계했고, 우선순위(Must·Should·Could·Won't)는 **MVP 컷라인(F1(성장 나무)~F13(소비 내역·전월 비교) ✔ / F14(3분 체류 위치 알림)~F19(소비 사전 개입 2차 트리거) ✖)을 유지**하는 방향으로 배치했습니다.
...
| **F8** | 💳 결제 내역 + 계획↔실제 회고 (+ **회고 문장 풀**) | ★ 선별 C5(3분 미달 사각지대) △ 감시 C6(회고 형식화) | J1(실천할 자리 만들기) J2(행동 변화 보여주기) | **5** | 4 | **Must** | 대체재는 **울린 뒤에 온다**. 「행동」 데이터의 사후 절반 |
...
| **F13** | 📊 소비 내역 (**전월 대비 증감액 상단**) | ★ 선별 C5(3분 미달 사각지대) | J2(행동 변화 보여주기) | 4 | 2 | **Should** | **H2의 최소 유지 조건** — 🧪 *"어디서 얼마 썼는지, 그거 하나만"* |
| **F19(소비 사전 개입 2차 트리거)** 🔴 | 🚦 **소비 사전 개입 2차 트리거** (체류 무관 · 온라인 포함) | ★**C5(3분 미달 사각지대)** ◇ 보조 C10(FOMO 3초 판단) ◇ 보조 C12(온라인 개입 부재) | J1(실천할 자리 만들기) | **5** | **5** | **Should** 🔴 **결정 대기 → §4-3** | **세 Pain을 한 번에 덮는 유일한 수단.** 대체재 사전 개입 수단 0개 |
...
| **F14** | 📍 위치 알림 (3분 체류 · 온디바이스) | ★ 선별 C5(3분 미달 사각지대) *(부분)* | J1(실천할 자리 만들기) | 4 | **5** | **Won't** *(v0.1)* | 🔴 **§4-2 참조** — 올려도 C5(3분 미달 사각지대)를 해결하지 않음 |
| **F18** | 📤 기존 앱 기록 이전 / 병행 사용 | (H1 Anxiety) | J3(끊김 방지) | 2 | **5** | **Won't** *(v0.1)* | 난이도 5 · 병행 수요 낮음(🧪 *"둘 다요? 귀찮을 것 같은데요"*) |

**집계** — Must **8** · Should **6**(F19(소비 사전 개입 2차 트리거) 포함) · Could **3** · Won't **2** = **19개**

### 4-2. 🔴 F14(3분 체류 위치 알림)를 Must로 올리지 않는 이유

F14(3분 체류 위치 알림) 제외 사유 넷 중 **셋은 그대로 유효**합니다.

| # | 사유 | 상태 |
|:-:|---|:-:|
| ① | **기술 구현 미검증** — 백그라운드 위치 권한 유지율 · 배터리 · 아동 기기 권한 | 🔴 유효 (검증과제 ⑥) |
| ② | **좌표 미저장 정책(P-19: 만 14세 미만 위치정보 옵트인·좌표 미저장)상 온디바이스 판정 필수** → 클라이언트 부담 큼 | 🔴 유효 |
| ③ | **3분 트리거가 잡아야 할 소비를 못 잡음** | 🔴 유효 — **이것이 곧 ★ 선별 C5(3분 미달 사각지대)** |
| ④ | ~~H6형에서 감시로 오해되어 1차 타깃 아닌 세그먼트를 끌어들임~~ | ✅ **소멸** — H6이 범위 밖으로 결정됨 |

> **결론** — ③이 C5(3분 미달 사각지대) 자체이므로 **F14를 승격해도 C5는 해결되지 않습니다.** 필요한 것은 같은 기능의 승격이 아니라 **다른 트리거(F19 소비 사전 개입 2차 트리거)** 입니다. 사유가 넷에서 셋으로 줄었다는 사실은 기록해 둡니다.

### 4-3. 🔴 최상위 결정 — F19(소비 사전 개입 2차 트리거)를 어디에 둘 것인가

**현재 배치: Should (v0.1 기준) · 결정 대기.** 아래 세 옵션 중 하나를 팀이 확정해야 합니다.

| 옵션 | 배치 | 장점 | 대가 |
|:-:|---|---|---|
| **A** *(현재)* | **Should** — MVP 밖 유지 | 선결 조건 4개가 미해결인 기능이 **MVP 일정을 잡지 않음.** C5(3분 미달 사각지대) 데이터는 F8(결제 회고)(사후 회고)로 절반 확보 | 🔴 **선별 3건 중 1건 미대응** · 사전 개입을 대외적으로 말할 수 없음 |
| **B** | **F19(소비 사전 개입 2차 트리거)를 쪼개서 F19(소비 사전 개입 2차 트리거)-a만 Must** — *아이 주도형 경량 트리거*(계획 카드 상시 노출 + [지금 살까?] 버튼) | 🟢 **선결 조건 4개를 전부 우회** — 위치 권한·좌표·제휴사 API 불필요, **키즈워치라도 무관.** 난이도 5 → **2~3 추정** | 자동 발동이 아니라 **아이가 열어야 함** → 효과가 약함(그 효과 자체가 검증 대상) · **MVP 범위 증가** |
| **C** | F19(소비 사전 개입 2차 트리거)를 **Must**로 승격 | 선별과 MVP가 정합 | 🔴 기술 검토·기기 전제·P-19(좌표 미저장 → 온디바이스 판정)가 **전부 미해결** — MVP가 이 하나에 블록될 위험 |

**선결 조건 (B·C 공통 · §7-3 의존성과 연결)**

1. 🔴 **아이 기기 전제 판정** — 키즈워치·부모 폰 공유에서 알림 수신이 가능한가 *(실기기 기술 검토)*
2. 🔴 **체류시간 실측** — C5(3분 미달 사각지대)의 발생률 0.6 재산출 *(스톱워치)*
3. **P-19 준수 방식** — 온디바이스 판정 구현 범위
4. **온라인 결제 개입 수단** — 카드사·제휴사 정책(P-17·P-18) 범위 확인

> ⚠️ **옵션 B의 F19(소비 사전 개입 2차 트리거)-a는 v13 확정 사양 안에서 파생됩니다** — 5단계 루프의 **①계획** 단계가 *"부모 또는 아이가 미리 정해둠"* 으로 이미 확정돼 있습니다 **[설계 확정]**. 없는 것은 **「그 계획을 소비 직전에 보게 하는 경로」** 뿐입니다. 다만 **F19(소비 사전 개입 2차 트리거)-a는 아직 확정 사양이 아니므로**, 채택 시 v13 개정이 함께 필요합니다.
...
| **사전 개입 수단 0개**(F14(3분 체류 위치 알림) Won't · F19(소비 사전 개입 2차 트리거) Should) | §4-2·§4-3에 사유 명시 · 대외 문구에서 **현재형 사용 금지** |
...
| **아동 계정** | id · 보호자_id · 생년(만 나이) · 기기 유형(전용폰/키즈워치/공유/없음) | 🔴 **기기 유형이 F14(3분 체류 위치 알림)·F19(소비 사전 개입 2차 트리거) 대상 판정에 쓰임** |
...
| *(내부)* 위치 판정 | 계획 장소 · 온디바이스 판정 결과 | 트리거 발동 이벤트 | 좌표 미전송(P-19) · **F14(3분 체류 위치 알림)·F19(소비 사전 개입 2차 트리거) 전제** |
...
| **Out** *(이번 릴리즈)* | 📍 위치 알림(F14) · 예적금 실천(F15) · 카드 없는 체험(F16) · 별의 옷장 외 목적지(F17) · 기록 이전(F18) · 🔴 **소비 사전 개입(F19(소비 사전 개입 2차 트리거) — 결정 대기)** |

### 7-2. 리스크 — VPS §9의 🔴 항목 우선

| # | 리스크 | 영향 | 완화 |
|:-:|---|---|---|
| **R1** 🔴 | **선별 3건 중 C5(3분 미달 사각지대)의 대응 기능이 MVP 밖** | 차별점 절반(사전 개입)을 말할 수 없고, *"왜 선별했나"* 라는 질문에 취약 | §4-3 결정을 **v0.2 전에** 확정 · F19(소비 사전 개입 2차 트리거)를 로드맵 1순위 고정 |
| **R2** 🔴 | **아이 기기 전제 미확정** — 선별 인물 둘 다 전용 폰 없음. 페르소나 문서(「키즈워치만」)와 여정 문서(알림 정상 작동)가 **어긋나 있음** | F14(3분 체류 위치 알림)·F19(소비 사전 개입 2차 트리거) 전체가 이 전제 위에 있음. 불가하면 **C5(3분 미달 사각지대) 발생률 0.6에 기기 보유율을 곱해 재산출** | **실기기 기술 검토를 최우선 선행**(§8-4) · 스크리너에 기기 유형 분류 |
...
| 유형 | 내용 | 확정 방법 | 상태 |
|---|---|---|---|
| 🔴 **의존성** | **아이 기기에서 알림 수신 가능 여부**(키즈워치·부모 폰 공유) | 실기기 기술 검토 | ⬜ **미확인 — F14(3분 체류 위치 알림)·F19(소비 사전 개입 2차 트리거) 블로커** |
...
| **Core 2** | **H2형** — 현금 용돈 + 고정 소비 동선 | 검증과제 ② · 🔴 **3분 체류가 실제 동선과 맞는가(C5 3분 미달 사각지대)** · 기기 유형 |
...
| **E3** 🔴 | 3분 체류가 실제 소비를 덮는다 | **스톱워치 실측** · 동선 3곳 × 5회+ | 3분 초과 비율 | **≥ 70%** | 50~69% | **< 50%** → C5(3분 미달 사각지대) 확정 · F19(소비 사전 개입 2차 트리거) 필수 · 발생률 0.6 재산출 |
...
| **E7** 🔴 | 아이 기기에 알림이 도달한다 | **실기기 검토** — 키즈워치 · 부모 폰 공유 | 수신 성공/시도 · 권한 유지율 · 배터리 증분 | 수신 **≥ 90%** · 24h 권한 유지 **≥ 90%** | 70~89% | **< 70%** → F14(3분 체류 위치 알림)·F19(소비 사전 개입 2차 트리거) 대상 범위 축소 + 대체 경로 설계 |
...
| **② 개입 시점** | **사전 개입 수단 개수** | 0개 | 0개 | 0개 | 🔴 **0개** *(F14(3분 체류 위치 알림)·F19(소비 사전 개입 2차 트리거) 제외)* → 로드맵 1개 | 기능 목록 대조 |
...
> 🔴 **②는 수치화해도 MVP에서 0 vs 0입니다.** 숨기지 않습니다 — ①③④에서 우위를 주장하고 **②는 "로드맵"으로만** 말합니다(§4-3 F19(소비 사전 개입 2차 트리거) 결정 대기).
...
```mermaid
flowchart LR
    A["① 기술 검토<br/>아이 기기 알림 수신<br/>(E7 · R2)"] --> B["② 체류시간 실측<br/>스톱워치 · 동선 3곳<br/>(E3 · C5(3분 미달 사각지대) 발생률)"]
    B --> C["③ F19(소비 사전 개입 2차 트리거) 배치 결정<br/>옵션 A/B/C<br/>(§4-3)"]
...
> **①②를 먼저 두는 이유** — 둘 다 **문항이 아니라 계측**이고, 결과가 F19(소비 사전 개입 2차 트리거) 배치(§4-3)와 C5(3분 미달 사각지대) 발생률을 직접 바꿉니다. 인터뷰를 먼저 하면 *"알림 반응"* 을 물어도 **알림이 도달하는지 자체를 모르는 상태**라 답이 무의미해집니다.
...
| C1(성장 증거 부재)·C2(실천 공백 → 나무 정체)가 DOS 양쪽 1위, C5(3분 미달 사각지대)가 AOS 1위다 | §7-6 | **[추론]** | **인터뷰 후 Imp·Sat 재채점**(8슬롯 완료 시) | 🔴 *"검증됨"으로 쓰지 않음* · **순위 변동 여부만** 판정 |
...
flowchart LR
    ...
    subgraph GAP["🔴 MVP 밖 — 사전 개입"]
        S2["② 트리거<br/>F14(3분 체류 위치 알림): 3분 체류 (Won't)<br/>F19(소비 사전 개입 2차 트리거): 2차 트리거 (Should·결정 대기)"]
    end

    S2 -.->|"🔴 MVP에서는 이 경로가<br/>존재하지 않음"| S3
    S1 ==>|"MVP 실제 흐름<br/>계획 → 소비 (개입 없음)"| S3

    S3["③ 소비<br/>아이 판단"] --> S4["④ 결제 이후<br/>계획 ↔ 실제 금액 비교<br/>(F8(결제 회고) · MVP ✔)"]
    S4 --> S5["⑤ 결제 회고<br/>앱이 문장 제공 → [확인했어요] → ⭐1"]
    S5 --> RISK["🔴 △ 감시 C6(회고 형식화) 위험<br/>읽지 않고 눌러도 ⭐1<br/>→ 회고 문장 풀 다양화로 완화"]
...
> 🔴 **★ 선별 C5(3분 미달 사각지대)의 정체가 이 그림입니다** — ①과 ③ 사이에 아무것도 없습니다. F14(3분 체류 위치 알림)를 넣어도 **자판기 30초 · 팽이 1분** 구간은 여전히 비어 있어, 필요한 것은 **다른 트리거(F19 소비 사전 개입 2차 트리거)** 입니다.
...
| **6** | 🔴 **최상위 결정 = F19(소비 사전 개입 2차 트리거) 배치(옵션 A/B/C)** — 선별 3건 중 C5(3분 미달 사각지대)가 MVP 미대응인 상태를 §1-4·§4-3·§7-2·§8-3에 네 번 남김 |
| **7** | 🔴 **선결 순서 = 기술 검토 → 체류시간 실측 → F19(소비 사전 개입 2차 트리거) 결정 → 프로토타입 → 인터뷰 → v0.2** — 앞의 둘은 문항이 아니라 **계측**이며, 인터뷰를 먼저 하면 답이 무의미해짐 |
...
| **2** | **② 개입 시점 축의 우위** | 수치화했으나 MVP에서 **0 vs 0** — 측정 문제가 아니라 **F19(소비 사전 개입 2차 트리거) 배치 결정(§4-3)** 이 열려 있습니다 |
