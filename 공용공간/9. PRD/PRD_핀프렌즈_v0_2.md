...
- **판본** — v0.1 → **v0.2**: `PRD_핀프렌즈_v0_1_품질리뷰.md`의 **측정 가능성·검증 가능성 결함 11건(CRITICAL 6)** 을 적용. 변경 요약은 [부록 E](#부록-e-v01--v02-변경-내역)
...
> 4. 🆕 **모든 인터뷰 지표는 `k/8` 정수 판정**이고, 모든 KPI·AC·실험은 **PASS / HOLD / FAIL 3구간**으로 판정합니다(§1-3-1 · §3-0).
...
> 🔴 **v0.1의 정의는 5종을 분자에 넣었으나, 그중 예적금(F15 Could)과 사전 개입(F19 Should)은 MVP에 없습니다.** 정의와 구현이 어긋나면 **WPA가 구조적으로 3종 상한에 묶인 것을 성능 문제로 오독**하게 됩니다.
...
| 선언 ② *(신뢰)* | 실천 근거 열람률 · 정체 원인 표시 열람률 | ⬜ | ≥ 60% | 월간 | `tree_view_opened.evidence_expanded` · `.stall_reason_shown` |
| J3 *(인정)* | 승인 지연 시 ⭐ 소급 지급 성공률 | — | **100%** | 주간 | `approval_state_changed(state=backfilled)` |
| J3 *(탐지)* | 아이 정지 → 부모 인지 일수 | ⬜ | ≤ 3일 | 주간 | `inactivity_notified` |
...
| 🆕 **AC-E1** *(F19 전제)* | **Given** 계획 금액이 **미설정** / **When** 트리거 조건에 도달한다 / **Then** **트리거를 발동하지 않고** 계획 설정을 유도한다 · **오탐 발동 0건** | 단위 테스트 |
| 🆕 **AC-E2** *(F19 전제)* | **Given** 아이 기기에서 **알림 권한이 거부**됨 / **When** 트리거 조건에 도달한다 / **Then** **앱 내 배너로 대체 노출**되고, **권한 거부 상태가 계측**된다 *(대상 범위 재산출 입력값)* | 권한 상태 로그 |
...
| 🆕 **AC-E2** | **Given** 온보딩 중 **세션 만료** / **When** 재로그인한다 / **Then** **직전 완료 단계에서 재개**되고, **동의(P-05) 단계는 재확인**한다 *(동의는 캐시하지 않음)* | 단위 테스트 |
...
| 데이터 타당성 | 회고 체류 중위 · 동일 문장 재노출률 | `retro_viewed.dwell_ms` p50 · `sentence_id` 중복률 | 중위 **< 3초** 또는 재노출 **> 2/8** | 콘텐츠 담당 | 1주 내 문장 풀 확장 | 2주 연속 → F8 스펙 재검토 |
...
| 🆕 **E10** 🔴 | **실천 증가가 실제 소비·저축 행동 변화로 이어진다** *(§9-1 [K]의 우리 제품 재현 검증)* | **3개월 종단 관찰** — 월간 숲 「지난달과 비교」 3지표 추적 | 사려다 멈춤 횟수 · 가격 비교 횟수 · 저축률 | **3지표 중 2개 이상**이 3개월 연속 개선 | 1개만 개선 | **실천은 늘고 3지표 불변** → 🔴 **선언 ①의 「성장」 재정의** |
...
| **2** | **② 개입 시점 축의 우위** | 수치화했으나 MVP에서 **0 vs 0** — 측정 문제가 아니라 **F19 배치 결정(§4-3)** 이 열려 있습니다 |
