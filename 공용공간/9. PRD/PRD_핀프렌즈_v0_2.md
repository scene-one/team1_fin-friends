...
| **F8** | 💳 결제 내역 + 계획↔실제 회고 **(두 갈래 — 지킴 ⭐1 / 넘김 회고만)** (+ **회고 문장 풀**) | ★ 선별 C5(3분 미달 사각지대) △ 감시 C6(회고 형식화) | J1(실천할 자리 만들기) J2(행동 변화 보여주기) | **5** | 4 | **Must** | 대체재는 **울린 뒤에 온다**. 「행동」 데이터의 사후 절반. **⭐가 「확인」이 아니라 「지킴」에 붙어야 나무·「변화 한 줄」의 원천 데이터가 참이 됨** |
...
flowchart LR
    ...
    S4 --> S5A["⑤-A 계획 지킴<br/>실제 ≤ 계획<br/>회고 → [확인했어요] → ⭐1"]
    S4 --> S5B["⑤-B 계획 넘김<br/>실제 > 계획<br/>회고 → [확인했어요] → ⭐ 없음<br/>(차감도 없음)"]
    S5A --> RISK["🟠 △ 감시 C6(회고 형식화) 잔여<br/>지킨 날 안 읽고 누름<br/>→ 회고 문장 풀 다양화로 완화"]
    S5B --> RISK2["🟠 신규 감시<br/>별이 없는 쪽 회고를<br/>안 열 수 있음 → US AC2-3"]
    ...
    style S5A fill:#e8f2e4,stroke:#4F7A4A
    style S5B fill:#f7eee6,stroke:#B36B3A
```
