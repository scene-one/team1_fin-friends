...
| **ADR-017** | **런타임 LLM·Vector DB를 쓰지 않는다** — 스택은 Next.js·Node/Express·PostgreSQL·Redis·AWS(서울)·three.js로 고정 (SRS §2.5) | MVP 기능 중 **런타임 LLM 호출이 필요한 것이 없다** — 학습 지문·퀴즈는 사전 제작, 미션 판정은 부모, ⭐ 판정은 금액 비교다. 그런데 아동 데이터를 국외 API로 보내면 **국외이전 없음(NFR-009)이 곧바로 깨지고**, 아바타는 P-13이 **생성형 AI 미사용**을 이미 못 박았다. AI/ML 연동 전용으로 제안된 Python·FastAPI도 근거가 함께 사라져 **런타임을 둘로 늘리는 비용만** 남는다. 🔑 지문·문항을 만드는 **사내 저작 도구**로 LLM을 쓰는 것은 아동 데이터가 나가지 않으므로 무관하며, 제품 스택이 아니다 | **국내 리전에서 처리 가능한 경로**가 생기고, 그것을 요구하는 기능이 로드맵에 올라올 때 |
