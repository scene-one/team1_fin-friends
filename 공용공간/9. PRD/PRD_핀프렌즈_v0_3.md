...
| **ADR-016** | **미션 승인 알림은 제출 즉시 1회** · 만료 전 **중간 리마인드 없음** | 리마인드는 같은 채널로 한 번 더 쏘는 것이라 **첫 알림이 안 닿은 부모에게는 두 번째도 안 닿는다** — PWA 푸시 도달률 자체가 `⬜ 미검증`이라 재발송은 병목을 푸는 수단이 아니다. 대기 건은 부모 화면 **승인 대기 목록**에 남고 5건 이상이면 일괄 승인 경로가 열린다 | `mission_expired` 비율 **> 10%**(SRS NFR-025) |
