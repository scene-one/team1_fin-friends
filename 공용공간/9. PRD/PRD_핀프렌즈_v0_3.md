...
- **하위 문서** — [`10. SRS/SRS_핀프렌즈_v1_0.md`](../10.%20SRS/SRS_%ED%95%80%ED%94%84%EB%A0%8C%EC%A6%88_v1_0.md) *(요구사항 명세 · FR/NFR/AC)*
...
> 3. **F8은 봉투형입니다** — 충전 시 용도별로 나눠 담고, 결제는 가맹점 업종에 따라 해당 봉투에서 **자동 차감**됩니다. 결제 건별로 무언가를 적게 하지 않습니다.
> 4. **「소비 순간 자동 개입」은 제공하지 않습니다.** 위치정보를 일절 쓰지 않습니다. 대외 자료에 **지오펜싱·위치 알림을 쓰지 마세요.**
...
| ◇ **C10(FOMO 3초 판단) · C12(온라인 개입 부재)** *(보조)* | 온라인 결제 사전 개입 | 🔴 **미대응 확정** — 대응 기능 **0개**. 온라인 결제는 가맹점 업종이 없어 **봉투에도 잡히지 않는다** |
...
### US-4 — 🧺 나눠 담은 대로 썼는지 맞춰본다  `J1 실천할 자리 만들기`  `★선별 C5 사전 개입 사각지대`
> **As a** 배주안(아이), **I want** 용돈을 용도별 봉투에 미리 나눠 담고 결제가 어느 봉투에서 빠졌는지 보고, **so that** 어디에 얼마를 썼는지 알고 다음에 조절할 수 있다.

| # | Given / When / Then | 측정 도구 · 표본 |
|:-:|---|---|
| **AC1** | **Given** 지갑 충전이 완료됐다 / **When** 아이가 배분 화면을 연다 / **Then** 용도별 봉투에 금액을 나눠 담을 수 있고, **합계가 충전액을 초과할 수 없다** | 단위 테스트 + 화면 검수 |
| **AC2** | **Given** 봉투가 배분된 아동 / **When** 카드 결제가 발생한다 / **Then** **가맹점 업종 코드로 봉투가 자동 결정**되고, 업종 매칭 정확도 **≥ 90%** | 결제 매칭 로그 · 수동 대조 표본 50건 |
| **AC3** | **Given** 결제가 발생 / **When** 회고 화면을 만든다 / **Then** 봉투 잔액 이내면 **⭐1**, 초과면 **⭐ 미지급**(보유 별 차감 없음)이고 **어느 봉투를 얼마나 넘겼는지** 표시된다 | 단위 테스트 |
| **AC4** *(생존 조건)* | **Given** MVP 운영 4주 / **When** 봉투 배분율을 집계한다 / **Then** **충전 건 대비 배분 완료율 ≥ 80%** *(O7)* | `envelope_allocated` ÷ `wallet_topped_up` |
| **AC-E1** | **Given** 봉투 잔액이 부족 / **When** 결제가 발생한다 / **Then** **결제는 통과**하고 ⭐만 미지급된다. 봉투 간 자동 이동은 하지 않는다 | 단위 테스트 |
| **AC-E2** | **Given** 결제 후 아이가 봉투 금액을 수정 / **When** ⭐ 판정을 조회한다 / **Then** **결제 시각 스냅샷 기준**이 유지되고 소급 변경되지 않는다 | 단위 테스트 |
| **AC-E3** | **Given** 어느 봉투에도 대응하지 않는 업종 코드 / **When** 결제가 발생한다 / **Then** **미분류 봉투**로 차감되고 부모에게 분류 요청이 표시된다 | 단위 테스트 |
...
### US-9 — 🧺 용돈을 용도별로 나눠 담는다  `J1 실천할 자리 만들기`  `★선별 C5 사전 개입 사각지대`

> **아이로서** 용돈을 받을 때 용도별로 나눠 담고 싶다. **어디에 얼마가 남았는지** 알고 쓰기 위해서다.

| AC | 내용 |
|---|---|
| **AC1** | 충전 완료 직후 배분 화면이 열리고, **충전 건 대비 배분 완료율 ≥ 80%** (O7 · MVP 4주 실측) |
| **AC2** | 결제 승인 시 **가맹점 업종에 대응하는 봉투에서 자동 차감**되고, 아이가 별도 입력을 하지 않는다 |
| **AC3** | 봉투 잔액 이내 결제는 **⭐1**, 초과 결제는 **⭐ 미지급**(보유 별 차감 없음) |
| **AC-E1** | 봉투가 비어도 **결제는 통과**한다. 초과 봉투명과 초과액이 부모 화면에 표시된다 |
| **AC-E2** | 결제 후 봉투 금액을 수정해도 **⭐ 판정은 결제 시각 스냅샷 기준**이며 소급되지 않는다 |
| **AC-E3** | 업종 코드가 어느 봉투에도 대응하지 않으면 **미분류 봉투**로 차감하고 부모에게 분류를 요청한다 |
...
| AC | 내용 |
|---|---|
| **AC1** | 아이가 **정액 비례 표현**(*"10,000원을 모으면 한 주에 ○○원"*)으로 제안하고, 부모 화면에는 **연 환산 이율이 함께** 표시된다 |
...
| **F8** | 🧺 **봉투형 소비 관리** — 충전 시 용도별 배분 · 결제 시 업종 기반 **자동 차감** · **두 갈래 회고**(봉투 안 ⭐1 / 초과는 회고만) · 금액 사후 수정 가능(⭐ 판정은 결제 시점 스냅샷 고정) | ★ 선별 C5(사전 개입 사각지대) △ 감시 C6(회고 형식화) | J1 J2 | **5** | **5** | **Must** | 사전(나눠 담기) + 사후(자동 대조)를 한 기능이 담당. **봉투 = 업종**이라 「어디에」가 입력 없이 데이터로 남음. 위치·알림·제휴사 한도 API에 의존하지 않아 **키즈워치 가정도 동작** |
...
| **부모 앱** | 성장 나무 · 월간 숲 · 미션 승인 · 소비 내역 · 봉투 열람 · 부모 적금 응답 · 온보딩·동의 |
| **아이 앱** | 옷장 홈 · 학습·퀴즈 · 실천하기 4칸 · 봉투 배분 · 부모 적금 신청 |
...
| **자동 사전 개입 0개** — 사전 수단은 **F8 봉투 배분**뿐 | 지오펜싱·위치 알림 서술 **대외 사용 금지**(사실 아님) · 생존 조건은 **봉투 배분율 ≥ 80%** |
...
| | 내용 |
|---|---|
| **In** | 앱으로 충전된 용돈의 흐름 — **벌기 → 잘 쓰기 → 모으기 → 불리기** · 학습 4주제 + 퀴즈(3단계) · 실천 판정(미션·봉투 대조·위시리스트·부모 적금 합의) · 별·옷장 · 성장 나무·월간 숲 · 소비 내역 · 부모 온보딩·동의 · 아이 앱 페어링 · 소급 지급 · 미접속 알림 · 미션 사진 확인(**서버 무저장**) |
| **Out** | **앱 외부에서 오간 현금**(세뱃돈·친척 용돈) · **친구 기능** · 모의투자 · **위치 기반 기능 일체** · **예적금 가입 중개** · 앱 내 협상(채팅·흥정) UI · 광고 · 아이템 현금 판매 |
...
| # | 리스크 | 영향 | 완화 | 감시 지표 |
|:-:|---|---|---|---|
| **R1** 🟠 | **봉투 배분율이 낮으면 사전·사후가 함께 무너진다** | 배분이 없으면 대조할 것이 없다 | 충전 완료 직후 배분 화면 강제 노출 | **O7 ≥ 80%** (E3) |
...
| **R8** | **아이 Pain 12건 중 부모 화면 신호 2건** *(봉투 초과·재배분)* | 나머지 10건은 여전히 안 보임 | **아이 인터뷰를 별도 과제로 등록** | 갈래 B 회고 열람률 ≥ 70% |
| **R9** 🔴 | **PWA 신뢰도** — 스토어에 없는 서비스에 아동 금융 정보를 맡길지가 `[가정]` | 1차 타깃이 아날로그 가정이라 특히 위험 | β 인터뷰 필수 문항 · 홈 화면 추가 안내 | 온보딩 이탈률 |
| **R10** 🔴 | **업종 매칭 미검증** — 모의 결제로는 정확도를 잴 수 없다 | 틀리면 **아이가 안 쓴 봉투에서 돈이 빠진다** | **가맹점 분류 상세도를 제휴사 선정 기준**으로 · 제휴 직후 최우선 검증 | **정확도 ≥ 90%** (US-4 AC2) |
...
| 🔴 **의존성** | **가맹점 업종 분류 상세도** | 제휴사 선정 기준 | ⬜ 미확인 — **봉투형의 성립 조건** |
...
| **가정** | 봉투 배분이 **결제 건별 작성보다 부담이 낮다** | β 4주 실측(O7) | 🔴 **[가설]** |
...
> **기능 목록은 VPS §8-2를 승계**했습니다. 🔄 **2026-08-21에 위치 기반 기능를 삭제하고 그 역할을 F8에 흡수**했으며(§4-2), 나머지 우선순위는 **MVP 컷라인(F1~F13 ✔ / F15~F18 ✖)을 유지**했습니다. 이것이 v0.2에서 유일하게 **기능 목록 자체를 바꾼 결정**입니다.
...
**집계** — Must **8** · Should **5** · Could **3** · Won't **1** = **17개** *(위치 기반 기능 삭제로 19 → 17)*
...
|---|---|---|
| **보호자 계정** | id · 인증정보 · 법정대리인 동의 상태/일시 · 알림 시간대 설정 | 동의 상태가 **아이 계정 활성화의 게이트** |
| **아동 계정** | id · 보호자_id · 생년(만 나이) · 기기 유형(전용폰/키즈워치/공유/없음) | 🔴 **기기 유형이 위치 기반 기능 대상 판정에 쓰임** |
| **학습 이수** | 아동_id · 주제(벌기/잘쓰기/모으기/불리기) · 완주 여부 · 퀴즈 정답 수 | 4주제 고정 |
| **실천 판정** | 아동_id · 경로(미션/회고/위시리스트/예적금/사전개입) · 판정 시각 · **판정 방식(자동/부모승인)** · 승인 지연 시간 | **WPA 산출의 원천 테이블** |
| **별 원장** | 아동_id · 증감 · 트리거 코드(1~8) · 잔액 · 사유 | 🔴 **이중 기입 · 초기화 없음 · 현금 전환 필드 부재** |
| **나무 상태** | 아동_id · 칸 · 단계 · 조건 충족(학습/퀴즈/실천) · 주기 시작일 · **정체 일수** | 정체 일수가 **F1(성장 나무) 정체 원인 표시**의 입력 |
...
| 유형 | 내용 | 확정 방법 | 상태 |
|---|---|---|---|
| 🔴 **의존성** | **아이 기기에서 알림 수신 가능 여부**(키즈워치·부모 폰 공유) | 실기기 기술 검토 | ⬜ **미확인 — 위치 기반 기능 블로커** |
...
| 4 | **「소비 순간 자동 개입」을 제공 기능**으로 | *"사전 수단은 봉투 배분뿐 — 자동 개입은 제공하지 않음"* |
...
### D. 소비 루프 — 🧺 사전 수단은 「봉투 배분」

```mermaid
flowchart LR
    S1["① 봉투 배분<br/>충전액을 용도별로 나눠 담음<br/>🍪 간식 · ✏️ 문구 · 🎯 모으기<br/>(F8 · MVP ✔)"] --> S2

    S2["② 소비<br/>봉투에 얼마 남았는지 알고 판단"] --> S3

    S3["③ 자동 대조<br/>가맹점 업종 → 해당 봉투에서 차감<br/>아이가 따로 입력하지 않음<br/>(F8 · MVP ✔)"]

    S3 --> S4A["④-A 봉투 안<br/>결제액 ≤ 봉투 잔액<br/>회고 → ⭐1"]
    S3 --> S4B["④-B 봉투 초과<br/>결제는 통과 · ⭐ 없음<br/>(보유 별 차감도 없음)<br/>→ 다음 배분 화면으로 연결"]

    S4A --> RISK["🟠 C6 잔여<br/>지킨 날 안 읽고 누름<br/>→ 회고 문장 풀 다양화"]
    S4B --> RISK2["🟠 신규 감시<br/>별이 없는 쪽 회고를<br/>안 열 수 있음 → US-5 AC2-3"]

    GAP["🔴 남은 사각지대<br/>온라인 결제 (C10 · C12)<br/>봉투에도 잡히지 않음"]
    S2 -.->|"대응 기능 0개"| GAP
...
> **사전 개입은 「결제 직전」이 아니라 「돈이 들어올 때」 일어납니다.** 아이가 매 결제 전에 앱을 열 필요가 없습니다.
> 🔴 **남은 사각지대는 온라인 결제입니다** — 오프라인 결제는 가맹점 업종으로 봉투에 잡히지만 온라인은 그렇지 않습니다. **대응 기능 0개**입니다.
> 🔑 **봉투 초과는 감춰지지 않고 신호가 됩니다** — *"🍪 간식 봉투를 1,200원 넘겼어요"* 가 부모 화면에 표시됩니다.
...
| **7** | 🔴 **선결 순서 = 기술 검토 → 체류시간 실측 → 위치 기반 사전 개입 결정 → 프로토타입 → 인터뷰 → v0.2** — 앞의 둘은 문항이 아니라 **계측**이며, 인터뷰를 먼저 하면 답이 무의미해짐 |
...
확정된 결정과 **그 결정을 그렇게 내린 이유**만 적습니다. 검토 경위와 기각안 근거는 지식베이스에 있습니다
(`wiki/` 각 페이지의 **「🧭 결정 배경 (Context)」** 절).

| ID | 결정 | 이유 | 되돌리는 조건 |
|---|---|---|---|
| **ADR-001** | **F8을 봉투형으로** — 충전 시 용도별 배분, 결제는 업종에 따라 자동 차감 | 결제 직전에 아이가 스스로 앱을 연다는 전제가 성립하지 않는다. 상기 수단을 바꿔도 전제는 안 바뀌므로 **적는 단위**를 바꿨다. 봉투 = 업종이라 대조가 입력 없이 성립하고, 제휴사 한도 API 의존도 사라진다 | O7 < 60%(E3 FAIL) → 배분 UX 재설계 |
| **ADR-002** | **봉투 초과 시 결제는 통과, ⭐만 미지급** | P-03(계획을 넘겨도 별을 차감하지 않는다)과 같은 철학. **미지급은 보상의 부재지만 결제 차단은 제재**다. 실제 차단은 제휴사 한도 API 종속이라 우리 권한도 아니다 | 갈래 B 회고 열람률 < 70% |
...
| **ADR-006** | **β는 제휴 없이 모의 결제로** | 봉투형이 제휴사 한도 API 의존을 없앴으므로 실결제 없이 학습·미션·봉투·나무·숲을 전부 검증할 수 있다. **β 결과 자체가 협상 카드**가 된다 | — |
...
| **ADR-011** | **이율은 부모가 정하고 우리는 기준선만 표시** | 우리가 숫자를 박는 순간 *"현실의 3배를 정답으로 가르친 책임"* 이 우리 것이 된다. 잘못된 기준선을 막는 장치는 이율 인하가 아니라 **화면에 시중 실측치를 함께 두는 것** | — |
